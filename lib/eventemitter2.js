// TODO:
// - Update comments.
// - "this._events" should be "this._listeners" because they are not events...
...
;!function(root) {
  ...
  EventEmitter2.prototype.emit = function(event) {
    ...
    if (listeners['all'] && listeners['all']._fn.length > 0){
      for (i = 0, l = listeners['all']._fn.length; i < l; i++) {
        listeners['all']._fn[i].apply(this, args);
        invoked = true;
      }
    }
  ...
  EventEmitter2.prototype.removeListener = function(event, listener) {
    if(listener && this._listeners[event] && this._listeners[event]._fn) {
      // Make a reference to all the listeners for the event.
      var listeners = this._listeners[event]._fn;
